...
export default async function BlogSingle({ params }) {
	...
	return (
		<article className='grid-12 mb-10 ck-content'>
			<Image image={postData.FeaturedImage} className='col--start-1 col--end-12 mb-6 max-h-[500px] overflow-hidden object-cover object-center' />
			<div className='col--start-4 col--end-9 container'>
				<h1 className='text-center'>{postData.Title}</h1>
				<h2 className='h3 text-center'>{postData.Summary}</h2>
				<div className='flex items-center justify-center text-center text-sm text-neutral-600 mt-8 mb-12'>
					...
					<span className='article-date'>{new Date(postData.createdAt).toLocaleString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })}</span>
					<span className="mx-3 block font-bold text-slate-500">·</span>
				...
				<HighlightedContentClient content={postData.Content} className='' />
			</div>
		</article>
	)
}