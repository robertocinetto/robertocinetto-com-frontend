...
async function getPostData(params) {
	...
	return postData.data[0] ? postData.data[0] : null
}


export async function generateMetadata({ params }, parent) {
	const postData = await getPostData(params)
	const postDataAttributes = postData.attributes

	const parentOpenGraph = await parent
	const parentOpenGraphImage = parentOpenGraph.openGraph?.images || []

	if (!postDataAttributes) return {}
	
	return {
		title: postDataAttributes.SEO?.MetaTitle,
		description: postDataAttributes.SEO?.MetaDescription,
		openGraph: {
			title: postDataAttributes.SEO?.MetaTitle,
			description: postDataAttributes.SEO?.MetaDescription,
			url: `https://robertocinetto.com/blog/${postDataAttributes.Slug}`,
			images: [postDataAttributes.SEO?.ShareImage ? getStrapiMedia(postDataAttributes.SEO.ShareImage) : null, ...parentOpenGraphImage],
		},
	}
}
...
export default async function BlogSingle({ params }) {
	...
	const wordCount = postDataAttributes.Content.split(" ").length
